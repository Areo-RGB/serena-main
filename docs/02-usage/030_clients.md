...
**Hooks.**
Due to recent changes (especially dynamic tool loading) in Claude Code, agents can drift back to repeated raw Read/Grep
calls even when Index MCP is available. These Serena-managed hooks now treat Index MCP's ``ide_*`` tools as the primary
code-intelligence layer and remind the agent to use them for code discovery/navigation.
...
- **`remind`**: Nudge the agent to use Index MCP tools such as `ide_find_symbol`, `ide_find_definition`,
  `ide_find_references`, `ide_file_structure`, and `ide_search_text` after repeated raw grep/read calls.
- **`activate`**: Tell the agent to use Index MCP as the primary discovery/navigation layer and initialize Serena when editing is needed.
- **`cleanup`**: Clean up hook session data when the session ends.
- **`auto-approve`**: Auto-approve Index MCP `ide_*` calls whenever Claude Code is in a permissive
  permission mode (`acceptEdits` or `auto`).
...
- **`activate`**: Tell the agent to prioritize Index MCP for discovery/navigation and initialize Serena when edits are needed.
- **`remind`**: Nudge the agent to use Index MCP `ide_*` tools after too many raw code-search/code-file-read calls.
- **`reset`**: Clear the reminder counters after a successful Index MCP `ide_*` call.
- **`cleanup`**: Clean up hook session data when the session ends.
...
Index MCP tool names, though, so it cannot also perform the counter reset when `ide_*` tools are used.
The separate `reset` hook above, matched to `PostToolUse` on `mcp__.*__ide_.*`, covers that case for Codex instead.
...
- **`remind`**: Nudge the agent to use Index MCP `ide_*` code-intelligence tools after too many
  consecutive raw code-search or code-file-read calls.
