...
from serena.hooks import (
    ...
    PreToolUseAutoApproveIndexMcpHook,
    PreToolUseHook,
...
class TestPreToolUseRemindAboutSerenaHook:
    ...
    def test_index_mcp_tool_detection(self, tmp_path: Path):
        for name, expected in [("mcp__index-mcp__ide_find_symbol", True), ("index_mcp.ide_file_structure", True), ("grep_search", False)]:
            with patch("sys.stdin", _make_stdin(_base_input(tool_name=name))), patch("serena.hooks.serena_home_dir", str(tmp_path)):
                hook = PreToolUseRemindAboutSymbolicToolsHook(HookClient.CLAUDE_CODE)
            assert hook.is_index_mcp_tool() == expected, f"is_index_mcp_tool() wrong for {name}"

    def test_index_mcp_tool_detection_grok_namespace(self, tmp_path: Path):
        payload = _grok_input("index-mcp__ide_find_symbol")
        with patch("sys.stdin", _make_stdin(payload)), patch("serena.hooks.serena_home_dir", str(tmp_path)):
            hook = PreToolUseRemindAboutSymbolicToolsHook(HookClient.GROK)
        assert hook.is_index_mcp_tool() is True
    ...
    def test_grok_index_mcp_tool_resets_counters(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """Grok Index MCP tool names use the ``index-mcp__ide_*`` namespace and reset raw grep/read counters."""
        session_id = "grok-reset"
        grep_payload = _grok_input("grep", {"pattern": "foo", "path": "."}, session_id=session_id)
        index_mcp_payload = _grok_input("index-mcp__ide_find_symbol", {"query": "Foo"}, session_id=session_id)

        for _ in range(ToolUseCounter._GREP_USES_THRESHOLD - 1):
            with patch("sys.stdin", _make_stdin(grep_payload)), patch("serena.hooks.serena_home_dir", str(tmp_path)):
                PreToolUseRemindAboutSymbolicToolsHook(HookClient.GROK).execute()

        with patch("sys.stdin", _make_stdin(index_mcp_payload)), patch("serena.hooks.serena_home_dir", str(tmp_path)):
            PreToolUseRemindAboutSymbolicToolsHook(HookClient.GROK).execute()
    ...
    def test_index_mcp_tool_resets_counters(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """Using an Index MCP tool should reset counters, so the threshold is not reached."""
        for _ in range(ToolUseCounter._GREP_USES_THRESHOLD - 1):
            with patch("sys.stdin", _make_stdin(_base_input("grep"))), patch("serena.hooks.serena_home_dir", str(tmp_path)):
                PreToolUseRemindAboutSymbolicToolsHook(HookClient.CLAUDE_CODE).execute()

        with patch("sys.stdin", _make_stdin(_base_input("mcp__index-mcp__ide_find_symbol"))), patch("serena.hooks.serena_home_dir", str(tmp_path)):
            PreToolUseRemindAboutSymbolicToolsHook(HookClient.CLAUDE_CODE).execute()
    ...
    def test_non_symbolic_deny_emitted_when_combined_threshold_tripped(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        ...
        # invoke execute with a neutral (non-grep, non-read, non-Index-MCP) tool so that
        ...
        hook_output = result["hookSpecificOutput"]
        assert hook_output["permissionDecision"] == "deny"
        assert "index mcp" in hook_output["additionalContext"].lower()
...
class TestToolUseCounter:
    ...
    def test_index_mcp_tool_resets_all_counters(self):
        counter = ToolUseCounter(
        ...
        hook = self._make_hook_stub("mcp__index-mcp__ide_file_structure", datetime.now())
        counter.update(hook)
...
class TestPreToolUseAutoApproveIndexMcpHook:
    """Tests for the auto-approve hook that allows Index MCP tools while the client is in a permissive permission mode (``acceptEdits`` or ``auto``)."""

    @staticmethod
    def _approve_input(
        tool_name: str = "mcp__index-mcp__ide_find_symbol",
    ...
    def test_approves_index_mcp_tool_in_accept_edits_mode(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """When the tool is an Index MCP tool and the mode is ``acceptEdits``, an allow decision is emitted."""
        stdin_data = self._approve_input()
        with patch("sys.stdin", _make_stdin(stdin_data)), patch("serena.hooks.serena_home_dir", str(tmp_path)):
            PreToolUseAutoApproveIndexMcpHook(HookClient.CLAUDE_CODE).execute()
    ...
    def test_approves_index_mcp_tool_in_auto_mode(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """When the tool is an Index MCP tool and the mode is ``auto``, an allow decision is emitted.

        Claude Code's ``auto`` mode is the hands-off-execution mode users adopt for autonomous
        runs; Index MCP tool calls should be auto-approved there for the same reason as in
        ``acceptEdits`` (Index MCP editing/refactoring calls could otherwise still prompt per call).
        """
    ...
    def test_stays_silent_for_non_index_mcp_tool(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """Non-Index-MCP tools get no decision even in ``acceptEdits`` mode (the hook stays silent)."""
    ...
    def test_stays_silent_in_default_mode(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """Index MCP tools in ``default`` mode get no decision (the hook stays silent)."""
...
class TestPostToolUseResetSymbolicToolCounterHook:
    ...
    def test_missing_session_id_raises(self, tmp_path: Path):
        stdin_data = {"tool_name": "mcp__index-mcp__ide_find_symbol", "tool_response": {"isError": False}}
        with patch("sys.stdin", _make_stdin(stdin_data)), patch("serena.hooks.serena_home_dir", str(tmp_path)):
            with pytest.raises(ValueError, match="Session ID is required"):
                PostToolUseResetSymbolicToolCounterHook(HookClient.CODEX)
    ...
    def test_resets_persisted_counter_on_successful_index_mcp_call(self, tmp_path: Path):
        ...
        payload = _post_tool_use_input("mcp__index-mcp__ide_find_symbol", session_id=session_id, tool_response={"isError": False})
        with patch("sys.stdin", _make_stdin(payload)), patch("serena.hooks.serena_home_dir", str(tmp_path)):
    ...
    def test_does_not_reset_on_failed_index_mcp_call(self, tmp_path: Path):
        """An Index MCP call that itself errored must not mask a real grep/read streak."""
        session_id = "reset-failure"
        seeded = ToolUseCounter(n_recent_grep_uses=2)
        self._seed_counter(tmp_path, session_id, seeded)

        payload = _post_tool_use_input("mcp__index-mcp__ide_find_symbol", session_id=session_id, tool_response={"isError": True})
        with patch("sys.stdin", _make_stdin(payload)), patch("serena.hooks.serena_home_dir", str(tmp_path)):
            PostToolUseResetSymbolicToolCounterHook(HookClient.CODEX).execute()
    ...
    def test_does_not_reset_without_a_tool_response(self, tmp_path: Path):
        ...
        payload = {"session_id": session_id, "tool_name": "mcp__index-mcp__ide_find_symbol"}
        with patch("sys.stdin", _make_stdin(payload)), patch("serena.hooks.serena_home_dir", str(tmp_path)):
            PostToolUseResetSymbolicToolCounterHook(HookClient.CODEX).execute()

        assert self._persisted_counter(tmp_path, session_id).n_recent_grep_uses == 2

    def test_does_not_reset_for_non_index_mcp_tool(self, tmp_path: Path):
    ...
    def test_does_not_reset_for_non_index_mcp_tool_namespace(self, tmp_path: Path):
        """A non-Index-MCP tool does not reset counters, even when its name comes from Serena."""
    ...
    def test_creates_fresh_counter_when_none_persisted_yet(self, tmp_path: Path):
        """A successful Index MCP call as the very first hook invocation of a session must not raise."""
    ...
    def test_codex_acceptance_scenario_successful_index_mcp_call_prevents_deny(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """The issue's own acceptance test: Bash(rg) -> Bash(rg) -> successful Index MCP call -> Bash(rg)
        must not deny the final call, using exactly the documented Codex hook wiring
        (``remind`` on PreToolUse/Bash, ``reset`` on PostToolUse/``mcp__.*__ide_.*``).
        """
        ...
        index_mcp_call = _post_tool_use_input("mcp__index-mcp__ide_find_symbol", session_id=session_id, tool_response={"isError": False})
        with patch("sys.stdin", _make_stdin(index_mcp_call)), patch("serena.hooks.serena_home_dir", str(tmp_path)):
            PostToolUseResetSymbolicToolCounterHook(HookClient.CODEX).execute()
    ...
    def test_codex_acceptance_scenario_failed_index_mcp_call_still_denies(self, tmp_path: Path, capsys: pytest.CaptureFixture[str]):
        """The issue's second acceptance test: a failed Index MCP call must not reset the streak,
        ...
        failed_index_mcp_call = _post_tool_use_input("mcp__index-mcp__ide_find_symbol", session_id=session_id, tool_response={"isError": True})
        with patch("sys.stdin", _make_stdin(failed_index_mcp_call)), patch("serena.hooks.serena_home_dir", str(tmp_path)):
            PostToolUseResetSymbolicToolCounterHook(HookClient.CODEX).execute()
...
class TestHookCli:
    ...
    def test_reset_command(self, tmp_path: Path):
        """The ``reset`` CLI command clears a persisted counter after a successful Index MCP call."""
        ...
        stdin_json = json.dumps({"session_id": session_id, "tool_name": "mcp__index-mcp__ide_find_symbol", "tool_response": {"isError": False}})
        runner = CliRunner()
        with patch("serena.hooks.serena_home_dir", str(tmp_path)):
    ...
    def test_reset_command_stays_silent_on_failed_call(self, tmp_path: Path):
        """The ``reset`` CLI command must not clear the counter for a failed Index MCP call."""
        ...
        stdin_json = json.dumps({"session_id": session_id, "tool_name": "mcp__index-mcp__ide_find_symbol", "tool_response": {"isError": True}})
        runner = CliRunner()
        with patch("serena.hooks.serena_home_dir", str(tmp_path)):
            result = runner.invoke(hook_commands, ["reset", "--client", "codex"], input=stdin_json)
        assert result.exit_code == 0
    ...
    def test_auto_approve_command(self, tmp_path: Path):
        """The ``auto-approve`` CLI command emits an allow for an Index MCP tool in acceptEdits mode."""
        stdin_json = json.dumps(
            {
                "session_id": "cli-auto-approve",
                "tool_name": "mcp__index-mcp__ide_find_symbol",
                "tool_input": {},
    ...
    def test_auto_approve_command_in_auto_mode(self, tmp_path: Path):
        """The ``auto-approve`` CLI command emits an allow for an Index MCP tool in ``auto`` mode."""
        stdin_json = json.dumps(
            {
        ...
        stdin_json = json.dumps(
            {
                ...
                "toolName": "index-mcp__ide_find_symbol",
                "toolInput": {},
                "permissionMode": "auto",
            }
        )
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("index-mcp__ide_find_symbol", "auto", {"decision": "allow"}),
            ("index-mcp__ide_find_symbol", "bypassPermissions", None),
            ("index-mcp__ide_find_symbol", "dontAsk", None),
            ("index-mcp__ide_find_symbol", "", None),
            ("grep", "auto", None),
        ],
        ids=["auto-allows-index-mcp", "bypass-silent", "dont-ask-silent", "empty-mode-silent", "non-index-mcp-silent"],
    )
    def test_auto_approve_command_grok_permission_modes(
        ...
        """Grok auto-approve is intentionally active only for Index MCP tools in ``auto`` mode."""
        stdin_json = json.dumps(
            {
